...
Browser field-length limits mirror the API limits, including 120 characters for name and destination, 180 for email, 80 for phone and quantity, 160 for dimensions, and 3,000 for project details. Every required field remains in the visible main form; the collapsed project-details section contains only optional controls. A visible note identifies required labels, while native validation keeps every invalid field marked with `aria-invalid` and reports the remaining count in the form status region. The endpoint accepts only JSON and URL-encoded form requests, and every normal form field must be a string; structured JSON values are rejected before delivery. Modern browser requests explicitly marked cross-site are rejected before body parsing. Production accepts only the exact `https://glorystarpacking.com` Origin; Vercel previews may use their exact HTTPS host, while the explicit local-development branch also supports its same HTTP(S) host. Requests without Fetch Metadata remain compatible with older browsers and the no-JavaScript fallback. Carriage returns and line feeds are removed from fields used in the email subject, and all three Resend environment variables—including the recipient—must be configured before delivery is attempted. Resend delivery is bounded by a 10-second server timeout, while the browser switches to the direct-contact fallback after 18 seconds. Identical normalized submissions use the same SHA-256 idempotency key so a network retry does not create a second email during the provider's deduplication window. A provider success response is accepted only when it contains valid JSON and a non-empty email ID; malformed success responses become a generic delivery failure instead of an unhandled server error.
...
- Submit an empty form and confirm every required field remains visible and highlighted, the first field receives native focus, and the status count decreases as fields are corrected.
- Confirm an explicitly cross-site JSON or URL-encoded browser request receives HTTP 403 without calling Resend; production accepts only the canonical HTTPS Origin, preview accepts only its same HTTPS host, and local development accepts its explicit same host.
