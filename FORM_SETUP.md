...
- Submit an empty form and confirm all five required fields remain highlighted, the first field receives native focus, and the status count decreases as fields are corrected.
- Confirm an explicitly cross-site JSON or URL-encoded browser request receives HTTP 403 without calling Resend, while canonical and same-host preview origins remain accepted.
