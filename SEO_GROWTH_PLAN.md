...
日常执行、任务优先级、30/60/90 天目标、发布闸门和复盘模板统一见 `DAILY_SEO_OPERATING_SYSTEM.md`。本文件继续维护页面与关键词分工、内容可信度边界和主题规划；日常运营不再以批量增加页面为默认目标。
...
| `/custom-packaging-quality-inspection-checklist.html` | packaging quality inspection checklist / packaging QC checklist |
...
- `/packaging-sample-approval-checklist.html` 负责量产前样品类型、适配、外观、工艺、包装方式和批准参考；`/custom-packaging-quality-inspection-checklist.html` 负责把已批准规格转成首件、过程、终检、外箱、不合格、整改、复检与出货放行记录，不生成通用抽样数或 AQL。
...
1. 已公开公司注册英文名、厦门所在地和业务联系主体，并在 About 页与 Organization 结构化数据中关联同公司运营的玻璃瓶站和阿里巴巴公司主页；后续仍需补充可公开的完整办公/工厂地址及注册资料核验方式；
2. 提供有使用权的工厂、设备、打样、质检和装箱实拍，并记录拍摄时间与环节；
3. 脱敏一份真实 QC 检验表或生产样批准表，保留尺寸、颜色、外观、功能和签核字段；
4. 如有证书，确认持证主体、编号、产品/站点范围、有效期与公开核验链接；
5. 取得客户授权后，再发布包含产品、约束、方案、验证方法和结果证据的案例；
6. 给每条产能、交期、MOQ、测试或可持续性声明建立内部证据来源与复核日期。

以上材料未确认前，页面只表达可核验的流程、采购输入和测试边界，不生成虚构工厂数据、证书或客户结果。
...
27. Custom packaging quality inspection checklist：把批准样、量产批次、检查阶段、特性、缺陷、抽样边界、外箱、不合格、整改、复检和出货放行做成受控记录（已于 2026-08-13 发布）。
...
SEO 不会在发布当天自动带来流量。第一阶段目标是让搜索引擎正确发现和理解当前 74 个英语可索引页面；第二阶段才是持续用真实内容、行业引用和可验证案例提升排名与转化。非英语页面必须先完成母语整页审核并进入 `translations/indexing.json` 白名单，不能用批量机器翻译扩大可索引 URL 数量。
